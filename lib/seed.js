...
function seedIfEmpty() {
  ...
  if (existing && existing.c > 0) {
    // DB already has data (saved via saveDb() on every write).
    // On Vercel (ephemeral filesystem) the DB file doesn't persist,
    // so we import from content.json. On local dev the DB is the source of truth.
