...
function createPackage(data) {
  return getDb().prepare(`INSERT INTO packages (slug, name, subtitle, price, price_label, features, pricing_json, is_popular, is_published, sort_order)
    VALUES (@slug, @name, @subtitle, @price, @price_label, @features, @pricing_json, @is_popular, @is_published, @sort_order)`).run(data).lastInsertRowid;
}

function updatePackage(id, data) {
  getDb().prepare(`UPDATE packages SET slug=@slug, name=@name, subtitle=@subtitle, price=@price,
    price_label=@price_label, features=@features, pricing_json=@pricing_json, is_popular=@is_popular,
