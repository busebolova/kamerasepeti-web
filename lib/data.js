...
function getAllPackages() {
  const packages = getDb().prepare('SELECT * FROM packages WHERE is_published = 1 ORDER BY sort_order ASC').all();
  return packages.map(p => ({
    ...p,
    pricing: p.pricing_json ? JSON.parse(p.pricing_json) : null
  }));
}
...
function createPackage(data) {
  return getDb().prepare(`INSERT INTO packages (slug, name, subtitle, price, price_label, features, pricing_json, category, is_popular, is_published, sort_order)
    VALUES (@slug, @name, @subtitle, @price, @price_label, @features, @pricing_json, @category, @is_popular, @is_published, @sort_order)`).run(data).lastInsertRowid;
}

function updatePackage(id, data) {
  getDb().prepare(`UPDATE packages SET slug=@slug, name=@name, subtitle=@subtitle, price=@price,
    price_label=@price_label, features=@features, pricing_json=@pricing_json, category=@category, is_popular=@is_popular,
